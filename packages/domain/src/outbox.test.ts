...
describe('sync outbox', () => {
  it('enqueues idempotently and claims a bounded leased batch', () => {
    ...
    const result = claimOutbox(outbox, now, 1)

    expect(outbox).toHaveLength(2)
    expect(result.claimed).toHaveLength(1)
    expect(result.claimed[0]).toMatchObject({ state: 'inflight', attempts: 1 })
    expect(
      claimOutbox(result.outbox, now).claimed.map((item) => item.event.id),
    ).toEqual(['evt-02'])
  })
  ...
  it('reclaims an event after its worker lease expires', () => {
    ...
    expect(
      claimOutbox(claimed.outbox, '2026-01-07T12:00:00.500Z').claimed,
    ).toHaveLength(0)
    expect(
      claimOutbox(claimed.outbox, '2026-01-07T12:00:01.000Z').claimed[0]
        .attempts,
    ).toBe(2)
  })

  it('retries with bounded exponential backoff and acknowledges delivery', () => {
    const claimed = claimOutbox(enqueueEvent([], event('evt-01'), now), now)
    ...
    expect(acknowledgeEvents(retried, ['evt-01'])).toEqual([])
  })
})
